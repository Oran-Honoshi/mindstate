...
export default function HexMergePage(){
  ...
  const loadStage=useCallback((s:number)=>{
    ...
    setXpState(xp);setCompleted(false);setFinalXP(0);setElapsed("00:00");setBestTile(0);
    ...
    if(selected===null){
      ...
      if(nc){
        ...
        if(checkHexWin(nc,target)&&xpState){
          ...
          if(user)saveScore({user_id:user.id,game_slug:"hex-merge",stage_number:stage,difficulty:getDifficulty(stage),xp_earned:earned,time_taken:Math.floor((Date.now()-xpState.startTime)/1000)});
        }
      } else {
  ...
  return(
    <div style={{minHeight:"100vh",background:"#FDFCFB",display:"flex",flexDirection:"column"}}>
      ...
      <main style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",padding:"76px 16px 32px",gap:18}}>
        <div style={{width:"100%",maxWidth:540,background:"white",borderRadius:20,border:"0.5px solid rgba(0,0,0,0.07)",padding:"16px 20px",boxShadow:"0 2px 8px rgba(0,0,0,0.04)"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <span style={{fontSize:12,color:"#94A3B8"}}>Best: {bestTile||"—"}</span>
        ...
        <div style={{fontSize:11,color:"#94A3B8"}}>Click a tile to select · Click a matching neighbor to merge · Reach {target}</div>
      ...
      <AnimatePresence>
        {completed&&(<motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",backdropFilter:"blur(12px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100,padding:24}}>
          <motion.div initial={{scale:0.9,y:20}} animate={{scale:1,y:0}} style={{background:"white",borderRadius:28,padding:36,maxWidth:340,width:"100%",textAlign:"center",boxShadow:"0 32px 80px rgba(0,0,0,0.2)"}}>
            ...
            <p style={{fontSize:13,color:"#94A3B8",marginBottom:24}}>{elapsed} · Best tile: {bestTile}</p>
            ...
            <button onClick={()=>shareResult(stage,finalXP,bestTile)} style={{width:"100%",marginBottom:12,padding:"11px",borderRadius:14,border:"0.5px solid #E2E8F0",background:"white",fontSize:13,fontWeight:600,color:"#374151",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}><Share2 size={14}/> Share Result</button>
