...
import { Search, Zap, Star, Trophy, Crown } from "lucide-react";
...
const GAMES = [
  { slug:"tango",         name:"Tango",           desc:"Balance rows & columns with equal Sun and Moon symbols",    free:true,  difficulty:"Easy–Hard", stages:100,  cat:"Logic"    },
  { slug:"memory",        name:"Memory",           desc:"Flip cards and find matching pairs before XP fades",       free:true,  difficulty:"Easy–Hard", stages:100,  cat:"Memory"   },
  ...
  { slug:"flow",          name:"Flow",             desc:"Connect color dots — fill every cell, no crossings",      free:false, difficulty:"Easy–Hard", stages:100,  cat:"Logic"    },
  ...
  { slug:"lightup",       name:"Light Up",         desc:"Place bulbs to illuminate every white cell exactly once", free:false, difficulty:"Easy–Hard", stages:100,  cat:"Logic"    },
  { slug:"nonogram",      name:"Nonogram",         desc:"Solve pixel art puzzles from row and column clues",       free:false, difficulty:"Easy–Hard", stages:100,  cat:"Puzzle"   },
  { slug:"pattern-match", name:"Pattern Match",    desc:"Identify the hidden rule and complete the sequence",      free:false, difficulty:"Easy–Hard", stages:100,  cat:"Puzzle"   },
  ...
  { slug:"gravity-sort",  name:"Gravity Sort",     desc:"Sort falling colored blocks into the correct columns",    free:false, difficulty:"Easy–Hard", stages:100,  cat:"Arcade"   },
  { slug:"hex-merge",     name:"Hex Merge",        desc:"Merge matching hexagonal tiles in chain reactions",       free:false, difficulty:"Easy–Hard", stages:100,  cat:"Arcade"   },
  ...
  { slug:"word-climb",    name:"Word Climb",       desc:"Change one letter at a time to reach the target word",   free:false, difficulty:"Easy–Hard", stages:100,  cat:"Word"     },
  { slug:"pinpoint",      name:"Pinpoint",         desc:"Guess the answer from cryptic clues",                    free:false, difficulty:"Easy–Hard", stages:100,  cat:"Word"     },
];

const TOTAL_STAGES = GAMES.reduce((sum, g) => sum + g.stages, 0);
...
export default function GamesPage() {
  ...
  const [stageMapGame, setStageMapGame] = useState<string | null>(null);
  const [cat, setCat] = useState("All");
  const [filter, setFilter] = useState("All");
  const [tokens, setTokens] = useState(FREE_DAILY_TOKENS);
  // Map of slug → completed stage count (for crown detection)
  const [completionMap, setCompletionMap] = useState<
    Record<string, { count: number; full: boolean }>
  >({});
  ...
  // Load completion state for all games (localStorage read, fast)
  useEffect(() => {
    const map: Record<string, { count: number; full: boolean }> = {};
    for (const g of GAMES) {
      const c = getCompletedStages(g.slug);
      map[g.slug] = {
        count: c.size,
        full: isGameFullyCompleted(g.slug, g.stages),
      };
    }
  ...
  const filtered = GAMES.filter((g) => {
    ...
    const matchFilter =
      filter === "All"
        ...
        ? g.free
        : filter === "Easy"
        ? g.difficulty.includes("Easy")
        : filter === "Hard"
        ? g.difficulty.includes("Hard")
        : true;
    return matchSearch && matchCat && matchFilter;
  });

  // Count how many games have ≥1 completed stage
  const gamesStarted = Object.values(completionMap).filter(
    (v) => v.count > 0
  ).length;
  const gamesMastered = Object.values(completionMap).filter(
    (v) => v.full
  ).length;
    ...
    <div style={{ minHeight: "100vh", background: "#FDFCFB", color: "#1C1917" }}>
      ...
      <main style={{ maxWidth: 1100, margin: "0 auto", padding: "76px 24px 60px" }}>
        ...
        >
          ...
          >
            {GAMES.length} games · {TOTAL_STAGES.toLocaleString()}+ stages.
            Free users get {FREE_DAILY_TOKENS} plays per day across all games.
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 11, color: "#94A3B8" }}>
                  Resets at midnight · Subscribe for unlimited
                </p>
              </div>
            </div>
                ...
                >
                  <div
                    style={{
                      height: "100%",
                      borderRadius: 2,
                      background: tokens > 0 ? ACCENT : "#EF4444",
                      width: `${(tokens / FREE_DAILY_TOKENS) * 100}%`,
        ...
        {isPro && (
          <motion.div
            ...
            style={{
              background:
                "linear-gradient(135deg,rgba(79,110,247,0.08),rgba(156,107,232,0.1))",
              borderRadius: 16,
          ...
          >
            ...
            <p style={{ fontSize: 13, fontWeight: 600, color: "#4F6EF7" }}>
              Pro — Unlimited plays on all {GAMES.length} games
            </p>
          </motion.div>
        )}
          ...
          {[
            {
              icon: Star,
              label: "Total Stages",
              value: `${TOTAL_STAGES.toLocaleString()}+`,
            },
            ...
            >
              ...
              <div>
                <p style={{ fontSize: 11, color: "#94A3B8", marginBottom: 1 }}>
                  {s.label}
                </p>
        ...
        {/* ── Progress summary (shown when user has played anything) ── */}
        {gamesStarted > 0 && (
          <motion.div
            ...
            style={{
              background:
                gamesMastered > 0
                  ? "linear-gradient(135deg,rgba(245,158,11,0.07),rgba(234,88,12,0.05))"
          ...
          >
            {gamesMastered > 0 ? (
              <Crown size={15} color="#F59E0B" fill="rgba(245,158,11,0.2)" />
            ) : (
              <Trophy size={15} color="#4F6EF7" />
            )}
            <p
              style={{
                fontSize: 13,
                fontWeight: 600,
                color: gamesMastered > 0 ? "#92400E" : "#4F6EF7",
              }}
            >
              {gamesMastered > 0
                ? `${gamesMastered} game${gamesMastered > 1 ? "s" : ""} mastered · ${gamesStarted} in progress`
                : `${gamesStarted} game${gamesStarted > 1 ? "s" : ""} in progress`}
            </p>
          </motion.div>
        )}
          ...
          {CATS.map((c) => (
            <button
              ...
              style={{
                ...
                boxShadow:
                  cat === c ? "0 4px 12px rgba(79,110,247,0.2)" : "none",
              }}
            >
              {c}
            </button>
        ...
        <div className="games-grid-4" style={{ marginBottom: 32 }}>
          {filtered.map((game, i) => {
            const completion = completionMap[game.slug];
            const isMastered = completion?.full ?? false;
            const stagesCompleted = completion?.count ?? 0;
            const progressPct = Math.min(
              100,
              Math.round((stagesCompleted / game.stages) * 100)
            );
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        {/* Crown badge for fully mastered games */}
                        {isMastered && (
                          <motion.div
                            initial={{ scale: 0 }}
                            animate={{ scale: 1 }}
                    ...
                    >
                      ...
                      {/* Progress bar (shown when player has started) */}
                      {stagesCompleted > 0 && (
                        <div style={{ marginTop: 8 }}>
                          ...
                          >
                            <span style={{ fontSize: 9, color: "#94A3B8" }}>
                              {stagesCompleted} / {game.stages} stages
                            </span>
                            <span
                              style={{
                                fontSize: 9,
                                fontWeight: 600,
                                color: isMastered ? "#F59E0B" : "#4F6EF7",
                              }}
                            >
                              {isMastered ? "Mastered" : `${progressPct}%`}
                            </span>
                          </div>
                          <div
                            style={{
                            ...
                            <motion.div
                              ...
                              style={{
                                ...
                                background: isMastered
                                  ...
                                  : "linear-gradient(90deg,#4F6EF7,#9C6BE8)",
                              }}
                            />
                          </div>
                        </div>
                      ...
                      <div
                        style={{
                          ...
                          alignItems: "center",
                          justifyContent: "space-between",
                          marginTop: stagesCompleted > 0 ? 8 : 10,
                          paddingTop: 8,
                          borderTop: "0.5px solid #F8F7F5",
                        }}
                      >
                        <span style={{ fontSize: 10, color: "#CBD5E1" }}>
                          {game.stages.toLocaleString()} stages
                        </span>
                        <span style={{ fontSize: 10, color: "#CBD5E1" }}>
                          {game.difficulty}
                        </span>
          ...
          >
            <div>
              ...
              <p style={{ fontSize: 13, color: "rgba(255,255,255,0.75)" }}>
                Unlimited plays · All {GAMES.length} games ·{" "}
                {TOTAL_STAGES.toLocaleString()}+ stages · Family leaderboards —
                from $2/mo
              </p>
            </div>
            <Link
      ...
      {stageMapGame && (
        <StageMap
          ...
          totalStages={
            GAMES.find((g) => g.slug === stageMapGame)?.stages ?? 1000
          }
          currentStage={1}
          onSelectStage={(s) => {
            window.location.href = `/games/${stageMapGame}`;
          }}
