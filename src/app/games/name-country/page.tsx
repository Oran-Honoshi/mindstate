...
function NameCountryInner(){
  ...
  useEffect(()=>{
    const saved=loadGameState("name-country");
  ...
  function submitGuess(){
    ...
    if(won){
      setTimeout(()=>{
        ...
        if(user){updateStreak(user.id);saveScore({user_id:user.id,game_slug:"name-country",stage_number:stage,difficulty:getDifficulty(stage),xp_earned:earned,time_taken:Math.floor((Date.now()-xpState.startTime)/1000)});}
  ...
  return(
    <div className="game-page">
      ...
      <main style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",padding:"76px 16px 32px",gap:16}}>
        ...
        {lost&&(
          <motion.div initial={{opacity:0}} animate={{opacity:1}}
            ...
            {board.flag} It was {board.answer} · Capital: {board.capital}
          </motion.div>
        )}
      ...
      <CompletionPopup open={completed} stage={stage} difficulty={getDifficulty(stage)} xpEarned={finalXP} elapsed={elapsed}
        ...
        onShare={()=>{const text=`MindState · Name the Country Stage ${stage} · ${finalXP} XP`;if(navigator.share)navigator.share({title:"MindState",text,url:"https://mindstate.vercel.app"}).catch(()=>{});else window.open("https://twitter.com/intent/tweet?text="+encodeURIComponent(text),"_blank");}}/>
    </div>
  );
}

export default function NameCountryPage(){
  return <ErrorBoundary game="name-country"><NameCountryInner/></ErrorBoundary>;
}
