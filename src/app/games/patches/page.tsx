...
export default function PatchesGame(){
  ...
  function handleCellClick(r:number,c:number){
    ...
    if(checkPatches(board,np)&&xpState){
      ...
      if(user)saveScore({user_id:user.id,game_slug:"patches",stage_number:stage,difficulty:getDifficulty(stage),xp_earned:earned,time_taken:Math.floor((Date.now()-xpState.startTime)/1000)});
    }
  }
    ...
    <div style={{minHeight:"100vh",background:"#FDFCFB",display:"flex",flexDirection:"column"}}>
      ...
      <main style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",padding:"76px 16px 32px",gap:18}}>
        ...
        <div style={{width:"100%",maxWidth:520}}>
          <p style={{fontSize:11,fontWeight:600,color:"#94A3B8",letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:10}}>Pieces</p>
          <div style={{display:"flex",flexWrap:"wrap",gap:10}}>
            {board.pieces.map(piece=>{
              if(placedPieces.has(piece.id))return(
                <div key={piece.id} style={{padding:8,borderRadius:14,border:"2px solid #E2E8F0",background:"#F8F7F5",opacity:0.4}}>
