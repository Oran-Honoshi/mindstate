...
export default function Pattern MatchPage() {
    ...
    <div style={{minHeight:"100vh",background:"#FDFCFB",display:"flex",flexDirection:"column"}}>
      ...
      <main style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"76px 24px 48px",gap:24}}>
        ...
        <motion.div initial={{scale:0.8,opacity:0}} animate={{scale:1,opacity:1}}
          style={{width:96,height:96,borderRadius:28,background:"#EDE9FE",display:"flex",alignItems:"center",justifyContent:"center",fontSize:48,boxShadow:"0 16px 40px rgba(0,0,0,0.1)"}}>
          ◧
        </motion.div>

        <div style={{textAlign:"center",maxWidth:400}}>
          <h1 style={{fontSize:28,fontWeight:700,color:"#1C1917",fontFamily:"Georgia,serif",marginBottom:8}}>Pattern Match</h1>
          <p style={{fontSize:15,color:"#64748B",lineHeight:1.65,marginBottom:24}}>Study the sequence, identify the hidden rule, and complete it. Gets fiendish fast.</p>
          ...
          <div style={{background:"white",borderRadius:20,border:"0.5px solid rgba(0,0,0,0.08)",padding:"20px 24px",marginBottom:20,boxShadow:"0 4px 16px rgba(0,0,0,0.06)"}}>
            ...
            <button onClick={()=>setShowPro(true)}
              ...
              Unlock Pattern Match
            </button>
          </div>

          {/* Preview */}
          <div style={{background:"#EDE9FE",borderRadius:20,padding:"20px",marginBottom:16,border:"0.5px solid rgba(0,0,0,0.06)"}}>
            <p style={{fontSize:11,fontWeight:600,color:"#7C3AED",letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:8}}>Coming in v1.1</p>
      ...
      <ProModal open={showPro} onClose={()=>setShowPro(false)} feature="Pattern Match"/>
    </div>
  );
}
