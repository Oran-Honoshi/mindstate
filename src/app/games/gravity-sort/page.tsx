  ...
  function handleColClick(col:number){
    ...
    if(selected===null){
      ...
      if(checkGravitySort(board,nb)&&xpState){
        ...
        if(user)saveScore({user_id:user.id,game_slug:"gravity-sort",stage_number:stage,difficulty:getDifficulty(stage),xp_earned:earned,time_taken:Math.floor((Date.now()-xpState.startTime)/1000)});
      }
    }
  }
        ...
        <div style={{width:"100%",maxWidth:520,background:"white",borderRadius:20,border:"0.5px solid rgba(0,0,0,0.07)",padding:"16px 20px",boxShadow:"0 2px 8px rgba(0,0,0,0.04)"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <span style={{fontSize:12,color:"#94A3B8"}}>Moves: {moves}</span>
              <span style={{fontSize:12,color:"#94A3B8",fontFamily:"monospace"}}>{elapsed}</span>
              <GameInstructions game="gravity-sort"/>
        ...
        <div style={{display:"flex",gap:gap}}>
          {board.colors.map((color,i)=>(
            <div key={i} style={{width:blockSize,textAlign:"center"}}>
              ...
              <span style={{fontSize:9,color:"#94A3B8"}}>col {i+1}</span>
            </div>
          ))}
          {Array.from({length:board.cols-board.colors.length},(_,i)=>(
            <div key={`empty-${i}`} style={{width:blockSize,textAlign:"center"}}>
              ...
              <span style={{fontSize:9,color:"#94A3B8"}}>free</span>
            </div>
          ))}
        </div>
