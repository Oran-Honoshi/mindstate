...
import{ArrowLeft,RotateCcw,ChevronRight,Share2}from"lucide-react";
    ...
    setTimeout(()=>{
      ...
      if(newHand.length===0){
        setTimeout(()=>{
          ...
          const won=fp<fc;
          if(won&&xpState){const earned=finalizeXP(xpState);setFinalXP(earned);playSuccess();setTimeout(()=>triggerConfetti(),80);markStageCompleted("hearts",stage);if(user){updateStreak(user.id);saveScore({user_id:user.id,game_slug:"hearts",stage_number:stage,difficulty:getDifficulty(stage),xp_earned:earned,time_taken:Math.floor((Date.now()-xpState.startTime)/1000)});}}
          else playError();
        },800);
      } else setPhase("play");
    },1200);
    ...
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#0F4C2A,#1A6B3A)",display:"flex",flexDirection:"column"}}>
      ...
      <main style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",padding:"76px 16px 32px",gap:16}}>
        ...
        <div style={{display:"flex",gap:12,alignItems:"center",flexWrap:"wrap",justifyContent:"center"}}>
          ...
          <ShowSolution onReveal={handleRevealSolution} currentXP={currentXP} disabled={phase==="done"||solutionRevealed}/>
        </div>
      ...
      {showMap&&<StageMap gameSlug="hearts" totalStages={1000} currentStage={stage} onSelectStage={s=>setStage(s)} onClose={()=>setShowMap(false)}/>}
    </div>
  );
}
export default function HeartsPage(){return<ErrorBoundary game="hearts"><HeartsPageInner/></ErrorBoundary>;}