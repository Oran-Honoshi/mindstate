...
export default function LightUpPage(){
  ...
  function handleCell(r:number,c:number){
    ...
    if(checkLightUp(ng)&&xpState){
      ...
      if(user)saveScore({user_id:user.id,game_slug:"lightup",stage_number:stage,difficulty:getDifficulty(stage),xp_earned:earned,time_taken:Math.floor((Date.now()-xpState.startTime)/1000)});
    }
  }
        ...
        <div style={{width:"100%",maxWidth:540,background:"white",borderRadius:20,border:"0.5px solid rgba(0,0,0,0.07)",padding:"16px 20px",boxShadow:"0 2px 8px rgba(0,0,0,0.04)"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <span style={{fontSize:11,color:"#94A3B8"}}>{litCount}/{totalWhite} lit</span>
