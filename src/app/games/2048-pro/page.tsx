...
export default function TwentyFortyEightProPage(){
  ...
  const handleMove=useCallback((dir:"up"|"down"|"left"|"right")=>{
    ...
    if(hasWon(ng,target)){
      ...
      if(user)saveScore({user_id:user.id,game_slug:"2048-pro",stage_number:stage,difficulty:getDifficulty(stage),xp_earned:earned,time_taken:Math.floor((Date.now()-xpState.startTime)/1000)});
    } else if(hasLost(ng)){
  ...
  return(
    <div style={{minHeight:"100vh",background:"#FAF8EF",display:"flex",flexDirection:"column"}}>
      ...
      <main style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",padding:"76px 16px 32px",gap:16}}>
        <div style={{width:"100%",maxWidth:400,background:"white",borderRadius:20,border:"0.5px solid rgba(0,0,0,0.07)",padding:"16px 20px",boxShadow:"0 2px 8px rgba(0,0,0,0.04)"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              ...
              <GameInstructions game="2048-pro"/>
        ...
        <div style={{fontSize:11,color:"#94A3B8",textAlign:"center"}}>Arrow keys or swipe to move · Reach {target} to win</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,48px)",gridTemplateRows:"repeat(2,48px)",gap:6}}>
          {[
            ...
            <button key={btn.dir} onClick={()=>handleMove(btn.dir as any)}
              style={{gridColumn:btn.col,gridRow:btn.row,width:48,height:48,borderRadius:12,border:"0.5px solid #E2E8F0",background:"white",fontSize:18,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 2px 6px rgba(0,0,0,0.06)"}}>
              {btn.label}
            </button>
          ))}
        </div>
