  ...
  return(
    <div style={{minHeight:"100vh",background:"#FDFCFB",display:"flex",flexDirection:"column"}}>
      ...
      <main style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",padding:"76px 16px 32px",gap:18}}>
        <div style={{width:"100%",maxWidth:540,background:"white",borderRadius:20,border:"0.5px solid rgba(0,0,0,0.07)",padding:"16px 20px",boxShadow:"0 2px 8px rgba(0,0,0,0.04)"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              ...
              <GameInstructions game="nonogram"/>
        ...
        {/* Mode toggle */}
        <div style={{display:"flex",background:"#F1EDE8",borderRadius:14,padding:3,gap:2}}>
          ...
          <div style={{display:"flex",marginLeft:CLUE_W}}>
            {board.colClues.map((clue,c)=>(
              <div key={c} style={{width:cellSize,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"flex-end",paddingBottom:4,minHeight:40}}>
                {clue.map((n,i)=>(
                  <span key={i} style={{fontSize:Math.min(cellSize*0.4,11),fontWeight:700,color:"#374151",lineHeight:1.3}}>{n}</span>
                ))}
              </div>
            ))}
          </div>
            ...
            <div key={r} style={{display:"flex",alignItems:"center"}}>
              ...
              <div style={{width:CLUE_W,display:"flex",justifyContent:"flex-end",alignItems:"center",gap:3,paddingRight:6,minHeight:cellSize}}>
                {board.rowClues[r].map((n,i)=>(
                  <span key={i} style={{fontSize:Math.min(12,cellSize*0.45),fontWeight:700,color:"#374151"}}>{n}</span>
                ))}
              </div>
              {/* Cells */}
                ...
                return(
                  <motion.button key={c} onClick={()=>handleCell(r,c)} whileTap={{scale:0.9}}
                    style={{width:cellSize,height:cellSize,display:"flex",alignItems:"center",justifyContent:"center",
                      ...
                      cursor:"pointer",outline:"none",fontSize:cellSize*0.4,color:"#94A3B8",fontWeight:700}}>
                    {val===false&&"✕"}
                  </motion.button>
                );
              })}
