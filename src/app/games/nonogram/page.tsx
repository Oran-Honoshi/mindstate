    ...
    <div style={{minHeight:"100vh",background:"#FDFCFB",display:"flex",flexDirection:"column"}}>
      ...
      <main style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"76px 24px 48px",gap:24}}>
        ...
        <motion.div initial={{scale:0.8,opacity:0}} animate={{scale:1,opacity:1}}
          style={{width:96,height:96,borderRadius:28,background:"#FCE7F3",display:"flex",alignItems:"center",justifyContent:"center",fontSize:48,boxShadow:"0 16px 40px rgba(0,0,0,0.1)"}}>
          ▪
        </motion.div>

        <div style={{textAlign:"center",maxWidth:400}}>
          <h1 style={{fontSize:28,fontWeight:700,color:"#1C1917",fontFamily:"Georgia,serif",marginBottom:8}}>Nonogram</h1>
          <p style={{fontSize:15,color:"#64748B",lineHeight:1.65,marginBottom:24}}>Solve pixel puzzles using row and column number clues. Reveal hidden pictures.</p>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
              {["1,000 stages","Daily challenges","XP leaderboard","Family groups"].map(f=>(
                <div key={f} style={{display:"flex",alignItems:"center",gap:6,fontSize:12,color:"#374151"}}>
                  <div style={{width:5,height:5,borderRadius:"50%",background:"#BE185D",flexShrink:0}}/>
                  {f}
                </div>
              ))}
            </div>
          ...
          {/* Preview */}
          <div style={{background:"#FCE7F3",borderRadius:20,padding:"20px",marginBottom:16,border:"0.5px solid rgba(0,0,0,0.06)"}}>
            <p style={{fontSize:11,fontWeight:600,color:"#BE185D",letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:8}}>Coming in v1.1</p>
      ...
      <ProModal open={showPro} onClose={()=>setShowPro(false)} feature="Nonogram"/>
    </div>
  );
}
