...
export default function LogicPathPage(){
  ...
  function handleRotate(r:number,c:number){
    ...
    if(checkLogicPath(ng)&&xpState){
      ...
      if(user)saveScore({user_id:user.id,game_slug:"logic-path",stage_number:stage,difficulty:getDifficulty(stage),xp_earned:earned,time_taken:Math.floor((Date.now()-xpState.startTime)/1000)});
    }
  }
  ...
  return(
    <div style={{minHeight:"100vh",background:"#FDFCFB",display:"flex",flexDirection:"column"}}>
      ...
      <main style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",padding:"76px 16px 32px",gap:18}}>
        ...
        <div style={{fontSize:11,color:"#94A3B8"}}>Click to rotate pipes · Connect all pipes so no end is open · 🔒 = locked</div>
