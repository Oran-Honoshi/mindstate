    ...
    <>
      ...
      >
        ...
        <div style={{
          ...
          background: theme === "dark" ? "rgba(6,13,24,0.85)" : "color-mix(in srgb, var(--color-surface) 80%, transparent)",
              ...
              {row.map((val, c) => {
                ...
                return (
                  <motion.button
                    ...
                    whileHover={!solutionRevealed && !isQueen && !isMark ? { filter: "brightness(1.08)" } : {}}
                    whileTap={!solutionRevealed ? { scale: 0.94 } : {}}
                    ...
                    style={{
                      ...
                      boxShadow: hasError
                        ? "inset 0 0 0 2px color-mix(in srgb, var(--color-error) 55%, transparent)"
                        : "none",
                    }}
                  >
                    {isMark && (
                        ...
                        animate={completed
                          ...
                          : {
                              ...
                              filter: hasError || isSolution
                                ? "drop-shadow(0 0 10px color-mix(in srgb, var(--color-error) 80%, transparent))"
                                : `drop-shadow(0 0 6px ${pal.queen})`,
                            }
                        }
                        transition={completed
                          ? { filter: { duration: 1.8, repeat: Infinity, ease: "easeInOut" }, scale: { duration: 0 }, y: { duration: 0 } }
      ...
      <CompletionPopup open={completed} stage={stage} difficulty={diff} xpEarned={finalXP} elapsed={finalElapsed}
        ...
        onShare={() => {
          const text = `Mind Element · Queens Stage ${stage} · ${finalXP} XP · ${finalElapsed}`;
