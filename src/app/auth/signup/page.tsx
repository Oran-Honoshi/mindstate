...
export default function SignUpPage() {
  ...
  if (success) return (
    <div className="auth-page" style={{ minHeight:"100vh", background:"var(--bg)", display:"flex", alignItems:"center", justifyContent:"center", padding:24 }}>
      <motion.div initial={{ opacity:0, scale:0.95 }} animate={{ opacity:1, scale:1 }}
        ...
        <h2 style={{ fontSize:20, fontWeight:700, color:"var(--text1)", fontFamily:"Georgia,serif", marginBottom:8 }}>Check your email</h2>
        <p style={{ fontSize:13, color:"var(--text3)", lineHeight:1.6 }}>
          We sent a confirmation link to <strong style={{ color:"var(--text1)" }}>{email}</strong>. Click it to activate your account.
        </p>
      </motion.div>
    </div>
  );
  ...
  return (
    <div className="auth-page" style={{ minHeight:"100vh", background:"var(--bg)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:24 }}>
      <motion.div initial={{ opacity:0, y:24 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.4 }} style={{ width:"100%", maxWidth:380 }}>
        ...
        <div className="ms-card" style={{ padding:"32px 28px" }}>
          <h1 style={{ fontSize:22, fontWeight:700, color:"var(--text1)", fontFamily:"Georgia,serif", marginBottom:4 }}>Create account</h1>
          <p style={{ fontSize:13, color:"var(--text3)", marginBottom:24 }}>Start training for free</p>
          ...
          <form onSubmit={handleSignUp}>
            {[
              ...
              <div key={field.label} style={{ marginBottom:14 }}>
                <label style={{ fontSize:11, fontWeight:600, color:"var(--text3)", letterSpacing:"0.1em", textTransform:"uppercase", display:"block", marginBottom:6 }}>{field.label}</label>
                <input type={field.type} value={field.value} onChange={e=>field.onChange(e.target.value)} required placeholder={field.placeholder}
                  style={{ width:"100%", padding:"11px 14px", borderRadius:12, border:"0.5px solid var(--border2)", fontSize:14, color:"var(--text1)", background:"var(--surface)", outline:"none" }}/>
              </div>
            ))}
            <div style={{ marginBottom:20 }}>
              <label style={{ fontSize:11, fontWeight:600, color:"var(--text3)", letterSpacing:"0.1em", textTransform:"uppercase", display:"block", marginBottom:6 }}>Password</label>
              <div style={{ position:"relative" }}>
                <input type={showPassword?"text":"password"} value={password} onChange={e=>setPassword(e.target.value)} required minLength={8} placeholder="min 8 characters"
                  style={{ width:"100%", padding:"11px 40px 11px 14px", borderRadius:12, border:"0.5px solid var(--border2)", fontSize:14, color:"var(--text1)", background:"var(--surface)", outline:"none" }}/>
                <button type="button" onClick={()=>setShowPassword(s=>!s)}
                  style={{ position:"absolute", right:12, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", cursor:"pointer", color:"var(--text4)" }}>
          ...
          <button onClick={handleGoogle}
            style={{ width:"100%", padding:"13px", borderRadius:14, border:"0.5px solid var(--border2)", background:"var(--surface)", fontSize:13, fontWeight:600, color:"var(--text2)", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
        ...
        <p style={{ textAlign:"center", fontSize:13, color:"var(--text4)", marginTop:20 }}>
