        ...
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          ...
          {user ? (
            ...
            <Link href="/auth/signin"
              style={{ fontSize:13, fontWeight:600, color:"var(--color-text-secondary)", textDecoration:"none",
                padding:"8px 14px", borderRadius:10, border:"0.5px solid var(--color-border)",
          ...
          <motion.div initial={{opacity:0,y:-8}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-8}}
            ...
            {NAV_LINKS.map(link=>(
              <a key={link.href} href={link.href} onClick={e=>handleScroll(e,link.href)}
                style={{ display:"block", fontSize:15, fontWeight:500, color:"var(--color-text-secondary)",
                  textDecoration:"none", padding:"10px 0", borderBottom:"0.5px solid var(--color-border)" }}>
                {link.label}
              </a>
            ))}
            <div style={{ display:"flex", gap:8, marginTop:12 }}>
              <Link href="/auth/signin" onClick={()=>setMobileOpen(false)}
                style={{ flex:1, textAlign:"center", padding:"11px", borderRadius:12,
                  border:"0.5px solid var(--color-border)", fontSize:13, fontWeight:600,
                  color:"var(--color-text-secondary)", textDecoration:"none", background:"var(--color-surface-2)" }}>
                Sign in
              </Link>
